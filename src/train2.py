...
if __name__ == '__main__':
    model = RTDETR('/mnt/rtdetr实验/RTDETR/ultralytics/cfg/models/rt-detr/rtdetr-CSP-PMSFA_RepNCSPELAN4_CAA_TransformerEncoderLayer_DPB.yaml')
    # model.load('') # loading pretrain weights
    model.train(data='data.yaml',
                cache=False,
                imgsz=640,
                epochs=1,
                batch=1, # batchsize 不建议乱动，一般来说4的效果都是最好的，越大的batch效果会很差(经验之谈)
                ...
                project='runs/train',
                name='rtdetr-CSP-PMSFA_RepNCSPELAN4_CAA_TransformerEncoderLayer_DPB',
                )