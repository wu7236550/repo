...
class Mamba2(nn.Module):
    ...
    def non_casual_linear_attn(self, x, dt, A, B, C, D, H=None, W=None):
        ...
        if self.ngroups == 1:
            KV = K.transpose(-2, -1) @ V_scaled
            Q = C.view(batch, 1, seqlen, dstate)
            x = Q @ KV
            ...
            K = K.view(batch, 1, seqlen, self.ngroups, dstate).permute(0, 1, 3, 2, 4)
            V_scaled = V_scaled.view(batch, head//self.ngroups, self.ngroups, seqlen, dim)
            Q = C.view(batch, 1, seqlen, self.ngroups, dstate).permute(0, 1, 3, 2, 4)

            KV = K.transpose(-2, -1) @ V_scaled
            x = Q @ KV
            V_skip = (V * D.view(1, -1, 1, 1).repeat(batch, 1, seqlen, 1)).view(batch, head//self.ngroups, self.ngroups, seqlen, dim)
            x = x + V_skip
            x = x.permute(0, 3, 1, 2, 4).flatten(2, 3).reshape(batch, seqlen, head, dim)
            x = x.contiguous()
