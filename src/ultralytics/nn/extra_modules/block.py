...
__all__ = ['Ghost_HGBlock', 'Rep_HGBlock', 'DWRC3', 'C3_DWR', 'C2f_DWR', 'BasicBlock_Ortho', 'BottleNeck_Ortho',
           ...
           'CAA_HSFPN', 'MCAF', 'C3_PKIModule', 'C2f_PKIModule', 'BasicBlock_FADC', 'BottleNeck_FADC', 'C3_FADC', 'C2f_FADC', 'C3_PPA', 'C2f_PPA', 'FocusFeature',
           ...
           'C2f_AdditiveBlock', 'C2f_AdditiveBlock_CGLU', 'CSP_MSCB', 'EUCB', 'C2f_MSMHSA_CGLU', 'MSPC', 'C2f_MogaBlock', 'C2f_SHSA', 'C2f_SHSA_CGLU', 'C2f_SMAFB', 'C2f_SMAFB_CGLU',
...
class CFC_CRB(nn.Module):
    ...
    def forward(self, x):
        ...
        query = self.query_conv(x).view(m_batchsize,32,-1).permute(0,2,1)
...
class ShiftConv2d1(nn.Module):
    def __init__(self, inp_channels, out_channels):
        ...
        self.n_div = 5
        g = inp_channels // self.n_div
        self.weight[0*g:1*g, 0, 1, 2] = 1.0
        self.weight[1*g:2*g, 0, 1, 0] = 1.0
        self.weight[2*g:3*g, 0, 2, 1] = 1.0
        self.weight[3*g:4*g, 0, 0, 1] = 1.0
        self.weight[4*g:, 0, 1, 1] = 1.0
...
class PSAttn(nn.Module):
    ...
    def __init__(self, dim, num_heads, topk=4, tau=1.0):
        ...
        self.gate_conv1d = nn.Conv1d(2 * head_dim, head_dim, kernel_size=1)
    ...
    def forward(self, x, upper_feat):
        ...
        if 0 < self.topk <= H_up * W_up:
            ...
            soft_weights = PSAttn.gumbel_softmax(global_sim)
            topk_weights, topk_indices = torch.topk(soft_weights, k=self.topk, dim=-1)
            ...
            gate = self.gate_conv1d(fusion_input)
            gate = torch.sigmoid(gate).transpose(1, 2).view(B, self.num_heads, N, self.head_dim)
            x = gate * refined_out + (1 - gate) * coarse_out
        else:
            x = coarse_out

        x = x.transpose(2, 3).reshape(B, self.all_head_dim, H, W)
